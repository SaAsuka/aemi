      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <form method="GET" className="flex gap-2">
          <input
            ...
            defaultValue={q}
            placeholder="案件名で検索"
            className="h-9 rounded-md border bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring w-48"
          />
          {status && <input type="hidden" name="status" value={status} />}
          <button type="submit" className="h-9 rounded-md border bg-background px-3 text-sm hover:bg-muted">検索</button>
        </form>
                ...
                {jobs.map((job) => (
                  <tr key={job.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link href={`/agency/jobs/${job.id}`} className="rounded-md border px-2.5 py-1 text-xs hover:bg-muted">
                        詳細
                      </Link>
                    </td>
                  </tr>
