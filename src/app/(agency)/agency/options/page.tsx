...
import { getAgencyOptions, getAgencyOptionCount } from "@/lib/actions/agency-option"
...
export default async function AgencyOptionsPage({
  searchParams,
}: {
  ...
  const [options, totalCount] = await Promise.all([
    getAgencyOptions(agency.id, q, status, page ? Number(page) : 1),
    getAgencyOptionCount(agency.id, q, status),
  ])
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <LinkButton href="/agency/options/new">新規作成</LinkButton>
      </div>
              ...
              {options.length === 0 ? (
                ...
                options.map((opt) => (
                  <TableRow key={opt.id}>
                    ...
                    <TableCell>
                      <LinkButton href={`/agency/options/${opt.id}`} size="sm" variant="outline">
                        詳細
                      </LinkButton>
                    </TableCell>
                  </TableRow>
