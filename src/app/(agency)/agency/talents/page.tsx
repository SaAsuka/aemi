...
import { getAgencyTalents, getAgencyTalentCount } from "@/lib/actions/agency-talent"
...
export default async function AgencyTalentsPage({
  searchParams,
}: {
  ...
  const [talents, totalCount] = await Promise.all([
    getAgencyTalents(agency.id, filters),
    getAgencyTalentCount(agency.id, filters),
  ])
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <LinkButton href="/agency/talents/new">新規登録</LinkButton>
        </div>
      </div>
            ...
            <TableBody>
              {talents.length === 0 ? (
                ...
                talents.map((talent, i) => (
                  <ClickableRow key={talent.id} href={`/agency/talents/${talent.id}`} className={i % 2 === 1 ? "bg-muted/30" : ""}>
                    <TableCell>
                      <Link
                        href={`/agency/talents/${talent.id}`}
                    ...
                    <TableCell>
                      <Link href={`/agency/talents/${talent.id}`}>
