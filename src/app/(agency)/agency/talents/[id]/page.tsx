...
import { getAgencyTalent } from "@/lib/actions/agency-talent"
...
import { AgencyTalentEditSheet } from "@/components/agency/agency-talent-edit-sheet"
...
export default async function AgencyTalentDetailPage({
  ...
  const talent = await getAgencyTalent(id, agency.id)
    ...
    <div className="max-w-4xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <AgencyTalentEditSheet talent={talent} />
          <AgencyDeleteButton id={talent.id} type="talent" />
        </div>
      </div>
          ...
          <dl className="grid grid-cols-1 gap-x-4 gap-y-2 text-sm sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            ...
            {(talent.bust || talent.waist || talent.hip) && (
              <div>
                ...
                <dd className="font-medium">{talent.bust ?? "−"}/{talent.waist ?? "−"}/{talent.hip ?? "−"}</dd>
              </div>
            )}
            {talent.category && (
              <div>
          ...
          <Table>
            ...
            <TableBody>
              {talent.applications.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} className="text-center text-muted-foreground">応募履歴がありません</TableCell>
                </TableRow>
              ) : (
