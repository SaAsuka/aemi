...
import { createAgencyJob, updateAgencyJob } from "@/lib/actions/agency-job"
...
function agencyJobAction(job?: Job) {
  return async (_prev: ActionResult, formData: FormData): Promise<ActionResult> => {
    if (job) {
      return await updateAgencyJob(job.id, formData)
    }
    return await createAgencyJob(formData)
  }
}
...
export function AgencyJobForm({
  ...
  const [state, action, isPending] = useActionState(agencyJobAction(job), null)
      ...
      <div className="border-t pt-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="space-y-2">
            ...
            <Input id="ageMin" name="ageMin" type="number" defaultValue={job?.ageMin ?? ""} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="ageMax">年齢（上限）</Label>
            <Input id="ageMax" name="ageMax" type="number" defaultValue={job?.ageMax ?? ""} />
          </div>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mt-4">
          <div className="space-y-2">
            <Label htmlFor="heightMin">身長（下限 cm）</Label>
            <Input id="heightMin" name="heightMin" type="number" defaultValue={job?.heightMin ?? ""} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="heightMax">身長（上限 cm）</Label>
            <Input id="heightMax" name="heightMax" type="number" defaultValue={job?.heightMax ?? ""} />
          </div>
        </div>
      </div>
      ...
      <div className="border-t pt-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input id="capacity" name="capacity" type="number" defaultValue={job?.capacity ?? ""} className="max-w-32" />
          </div>
        </div>
                    ...
                    <div className="space-y-1">
                      ...
                      {refFiles[cat] ? (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <button
                            type="button"
                            onClick={() => setRefFiles((prev) => { const next = { ...prev }; delete next[cat]; return next })}
                        ...
                        <Input
                          ...
                          onChange={(e) => { const file = e.target.files?.[0]; if (file) handleRefFileUpload(cat, file) }}
                          className="text-sm"
                        />
                      )}
