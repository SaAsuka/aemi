...
import { createAgencyOption, updateAgencyOption } from "@/lib/actions/agency-option"
...
function agencyOptionAction(option?: Option) {
  return async (_prev: ActionResult, formData: FormData): Promise<ActionResult> => {
    if (option) return await updateAgencyOption(option.id, formData)
    return await createAgencyOption(formData)
  }
}

export function AgencyOptionForm({ option }: { option?: Option }) {
  const [state, action, isPending] = useActionState(agencyOptionAction(option), null)
    ...
    <form action={action} className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          {state?.error?.status && <p className="text-sm text-destructive">{state.error.status[0]}</p>}
        </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="description" name="description" defaultValue={option?.description ?? ""} rows={4} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          {state?.error?.price && <p className="text-sm text-destructive">{state.error.price[0]}</p>}
        </div>
        <div className="space-y-2">
            ...
            defaultValue={
              option?.deadline
                ? (() => {
                    ...
                    if (jst.getUTCHours() === 23 && jst.getUTCMinutes() === 59) return datePart
      ...
      <div className="space-y-2">
        ...
        <Input id="sortOrder" name="sortOrder" type="number" defaultValue={option?.sortOrder ?? 0} className="max-w-32" />
      ...
      <div className="space-y-2">
        ...
        {imageUrl ? (
          <div className="space-y-2">
            <img src={blobProxyUrl(imageUrl, true)} alt="オプション画像" className="w-48 h-48 object-cover rounded-lg border" />
      ...
      {state?.success && option && <p className="text-sm text-green-600">更新しました</p>}
